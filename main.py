...
ADMIN_TOKEN = "admin_static_token"  # legacy static (kept for compatibility)
...
def grab_bonus(username: str = Form(...)):
    ...
    Bonus ₵ = 10% of invested amount (as per original logic). Bonus goes to withdrawable balance.
    """
    u = users.get(username)
...
def admin_logout(authorization: str = Header(None)):
    """Invalidate a dynamic admin token (if present)."""
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Missing or invalid token")
    token = authorization.split()[1]
    if token in ADMIN_TOKENS:
...
def admin_refresh(authorization: str = Header(None)):
    ...
    Returns both 'token' and 'new_token' in response to satisfy different frontends.
    """
    if not authorization or not authorization.startswith("Bearer "):
        raise HTTPException(status_code=401, detail="Missing or invalid token")
    ...
    if old_token in ADMIN_TOKENS:
        ...
        return {"message": "Admin token refreshed", "token": new_token, "new_token": new_token}
...
def approve_investment(username: str = Form(...), _: bool = Depends(admin_auth)):
    ...
    # mark investment approved
    inv["approved"] = True

    # add invested amount to user's principal (locked)
    u = users.get(username)
    if not u:
        raise HTTPException(status_code=404, detail="User not found")
    ...
    # set bonus days based on amount (existing logic)
    u["bonus_days_remaining"] = 30 if amt < 1000 else 60 if amt < 3000 else 90

    # referral bonus goes to ref's withdrawable balance & earnings
    ref = u.get("referral")
