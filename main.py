...
# ✅ UPDATED FRONTEND URL (CHANGE THIS ONCE ONLY)
...
# ---------------- COMMODITIES (UPDATED TO MATCH FRONTEND) ----------------
COMMODITY_INFO = {
    ...
    "growth_hub": {"price": 3000, "daily_bonus": 300, "expiry_days": 30},
    "wealth_builder": {"price": 3500, "daily_bonus": 350, "expiry_days": 32},
    "empire": {"price": 4000, "daily_bonus": 400, "expiry_days": 35},
}
...
# (ALL OTHER ADMIN ROUTES UNCHANGED — omitted for safety of structure)
...
def grab_bonus(data: GrabBonusRequest, current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    last_credited = datetime.fromisoformat(inv.get("last_credited", (now - timedelta(days=2)).isoformat()))

    if (now - last_credited).total_seconds() < 86400:
        raise HTTPException(status_code=400, detail="Wait 24 hours for next bonus")
    ...
    return {"message": f"Bonus KES {earned} claimed", "earned": earned}

# ---------------- WITHDRAW (UNCHANGED LOGIC) ----------------
...
def request_withdrawal(data: WithdrawRequest, current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    return {
        "message": f"Withdrawal request submitted. Net: KES {net_amount:.2f}"
    }

# ---------------- AUTH (UPDATED FRONTEND LINKS ONLY) ----------------
@app.post("/register")
...
def dashboard(current_user: UserDB = Depends(get_current_user)):
    return {
        ...
        "referral_bonus_earned": current_user.referral_bonus_earned,
        "investments": current_user.investments or {},
